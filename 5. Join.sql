-- 1. Modify it to show the match id and player name for all goals scored by Germany
...
SELECT matchid, mdate, COUNT(matchid) FROM game JOIN goal ON matchid = id WHERE team1 = 'POL' 
OR team2 = 'POL' GROUP BY matchid, mdate

-- 12. For every match where 'GER' scored, show matchid, match date and the number of goals scored by 'GER'
SELECT matchid, mdate, COUNT(player) FROM goal JOIN game ON (matchid = id) WHERE teamid = 'GER'
